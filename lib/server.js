...
const LEADERBOARD_MAX_LIMIT = 16 * 1024;
...
RouteHandler.prototype.run = function run() {
  ...
  if (req.method === 'GET') {
    ...
    match = req.url.match(/^(?:\/v1)?\/leaderboard(?:\?limit=(\d+))?$/);
    if (match !== null) {
      return this.getLeaderboard(
          match[1] !== undefined && parseInt(match[1], 10));
    }

    match = req.url.match(/^\/v1\/(tx|block)\/([a-f0-9]{64})$/);
    if (match !== null) {
      if (match[1] === 'block')
...
RouteHandler.prototype.getHelp = function getHelp() {
  this.respond(200, {
    ...
    '/help': 'this message',
    '/leaderboard': 'list of top currently unspent transactions',
...
RouteHandler.prototype.getLeaderboard = function getLeaderboard(limit) {
  ...
  this.server.chain.getUnspentTXs(limit, (err, txs) => {
    if (err)
